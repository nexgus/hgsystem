...
Guidance for Claude Code working in this repository. Build / deployment procedure lives in [README.md](README.md); this file is for things that aren't obvious from reading the code.
...
- `deprecated/` — old PySide2 code. Reference only; **do not add features here**.

## Invariants

These must not be changed without migrating data first:

- Mongo DB name `hgsystem`, collections `customers` / `worksheets` / `search`.
...
- Self-update flow (`SystemService.Update` + `更新` menu) does `git pull` + `go install ./cmd/hgsystem` + `syscall.Exec`. Target machines therefore need a Go toolchain to self-update.
